---
title: "Mozambique Sensitivity Analyses"
author: "Emily Decker"
date: "`r Sys.Date()`"
...
sens_data <- read_excel(model_inputs, sheet = "moz_point_estimates")

#Create icer function
calculate_icer <- function(df) {
  df$c_test = df$c_pd+df$c_travel+df$c_sup
...
icer_local <- calculate_icer(sens_data_scenario)
...
```{r local scenario analysis, include = FALSE}
sens_data_scenario <- read_excel(model_inputs, sheet = "moz_point_estimates_local") #for local lab testing scenario analysis

#Create icer function
calculate_icer_local <- function(df) {
  df$cost = df$accept*(df$c_test+df$c_treat * ((df$preval*df$sens)+((1-df$preval)*(1-df$spec))))
  df$effect = df$accept*df$preval*df$sens
...
icer_local <- calculate_icer_local(sens_data_scenario)
...
model_ranges <- sens_data_edit %>%
  ...
  mutate(min=ifelse(parameter=="preval", 0.002, min),
         max=ifelse(parameter=="preval", 0.1, max),
...
icer_plot_accept <- icer %>%
  ...
  select(study_arm,  incremental_cost, incremental_effect, icer) %>%
...
icer_plot_ccom <- icer %>%
  filter(study_arm!="grading") %>%
  select(study_arm,  incremental_cost, incremental_effect, icer) %>%
  mutate(incremental_cost = 0, incremental_effect = 0, icer = 0) %>% 
  mutate(study_arm=if_else(study_arm=="swab_dbs", "Swabs and DBS", study_arm),
         study_arm=if_else(study_arm=="swab", "Swabs", study_arm),
         study_arm=if_else(study_arm=="dbs", "DBS", study_arm)) %>%
  mutate(bound = "Base Value",
...
dsa_plot <- dsa_final %>%
  filter(study_arm!="grading") %>%
  select(study_arm, bound, param,  incremental_cost, incremental_effect, icer) %>%
...
```{r analyze_psa}
#Quantile regression on test sensitivity and per diems

psa_final$ln_sens = log(psa_final$sens)
psa_final$abs_icer = abs(psa_final$icer)

taus <- c(0.1,0.5,0.9)
rqfit <-rq(abs_icer ~ ln_sens + c_pd, data=psa_final%>%filter(study_arm!="grading"), tau = taus)
summary(rqfit)

#Probabilistic Sensitivity Analysis Cost-Effectiveness Plane (Figure 4)
